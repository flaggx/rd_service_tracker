...
const TicketSchema = z.object({
    ...
    // Accept empty string in UI as "unset"
...
export default function CreateTicketForm({ onCreated }) {
    ...
    // Drag & drop previews (UI only; uploads not yet wired)
    ...
    function buildPayload() {
        const payload = {
            ...
            priority: form.priority || undefined, // already uppercase from UI
    ...
    function mapServerErrorsToFields(serverErrors = []) {
        ...
        for (const er of serverErrors) {
            ...
            // Strip wrapper keys (body., params., query.)
            let key = rawPath.replace(/^body\./, "").replace(/^params\./, "").replace(/^query\./, "");
            // If it's an array path like pictures.0 -> assign to "pictures"
            if (key.startsWith("pictures.")) key = "pictures";
            // Only set if not set already; prefer first message for the field
    ...
    async function handleSubmit(e) {
        ...
        // Warn about dropped files (previews only)
        if (droppedFiles.length > 0 && (!pictures || pictures.length === 0)) {
            setNotice({
                type: "warn",
                text: "You added files via drag-and-drop, but uploads aren’t enabled yet. Add picture URLs above or proceed without images."
            });
        }
        ...
        try {
            ...
            if (err?.status === 400 && Array.isArray(err?.body?.errors)) {
                ...
                setNotice({
                    type: "error",
                    text: err?.body?.message || "Validation error"
                });
            } else {
    ...
    // Drag & drop handlers (UI previews only)
    ...
    function handleDrop(e) {
        ...
        if (images.length > 0) {
            ...
            setNotice({
                type: "info",
                text: "Images added locally for preview. Uploads are not yet wired; add URLs above or we can wire uploads next."
            });
        }
    }
    ...
    return (
        <form onSubmit={handleSubmit} style={{ border: "1px solid #ddd", padding: 12, marginBottom: 16 }}>
            ...
            {/* Drag & Drop area for future uploads */}
            <div
            ...
            >
                <div style={{ marginBottom: 8, fontWeight: 600 }}>Drag & drop images here (preview only)</div>
                <div style={{ fontSize: "0.9em", color: "#555", marginBottom: 8 }}>
                    Uploads are not yet wired. Add image URLs above for now. We can connect this dropzone to an upload API to generate URLs automatically.
                </div>
